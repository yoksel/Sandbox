...
/**
 * LJ Live iframe functionality
 * Template: templates/Discovery/Times.tmpl
 */
/* ---------------------------------------------------------------------------------
...
;(function () {
  ...
  angular.module('LJLive', ['LJ.Directives', 'LJ.RadioGroup', 'LJ.Api', 'LJ.User', 'Users'])
    .controller('LJLiveCtrl', ['$scope', '$timeout', '$interval', 'radioGroup', 'Api', 'Users',
                     function ( $scope,   $timeout,   $interval,   radioGroup,   Api,   Users ) {

      var active  = radioGroup('active'),
          timeout = 5000,

          query = LiveJournal.parseGetArgs(),

          // get journalid, entryditemid
          params = {
            journalid:    Number(query.journalid),
            entryditemid: Number(query.entryditemid)
          },
          origin = query.origin,
          slide = -1,
          timeoutId;

      $scope.active = active.reset().models();

      // filter incorrect items that could come from server
      $scope.items = LJ.get('items').filter(Boolean);

      Users.Cache.add(
        $scope.items.map( LJ.Function.get('user') )
      );

      $scope.showRecommend = LJ.get('showRecommend');
      $scope.isRecommended = LJ.get('isRecommended');
      $scope.showBranding  = angular.isDefined(query.branding);

      $scope.recommend = function () {
        Api.call('discovery.recommend', params);
      ...
      function nextSlide() {
        ...
        if (slide >= $scope.items.length) {
          slide = 0;
        }

        active.on( $scope.items[ slide ].post_id );
      }

      // send loading message to parent script
      sendMessage('ljlive ready');
